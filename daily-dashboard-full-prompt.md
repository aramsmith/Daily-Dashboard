...
- Versioning: one version number in MAJOR.MINOR.PATCH form (this release: 1.1.1), kept in a single VERSION file. The build, the setup, the uninstaller, the Windows Settings → Apps entry and the board all read it from there; the build stops if it is not in that form. Show it in the board's footer ("Daily Board 1.1.0"), on the Settings page, in the setup window title, and on the setup's Details page ("You have version X. This updates it to version Y.", "already installed", or a warning when installing an older version over a newer one) and Done page. Keep a CHANGELOG.md with what changed in each version.
...
- Stop the background service automatically after 30 minutes without any request. Every open board, briefing and draft page sends a small keep-alive request every 4 minutes, also from a background tab, so the service stops only when no board page is open. Count the 30 minutes in the service's own running time, so a PC that wakes up from sleep does not stop the board.
- When a page finds the service stopped, say so plainly and show a "Start the board" button. The setup registers a per-user dailyboard:// link that runs the board's own launcher with a fixed quiet option (it passes nothing from the link on and opens no extra tab); the page then waits until the service answers and reloads its data. The uninstaller removes the link.
